...
with torch.no_grad():
    true_y = odeint(Lambda(), true_y0, t,atol=1e-2, method='adaptiveLobatto',options={'step_size':0.1,'theta':0.7})
...
for itr in range(1, niters + 1):
    ...
    pred_y = odeint(func, batch_y0, batch_t ,atol=1e-2, method='adaptiveLobatto',faster_adj_solve=True,options={'step_size':0.1,'theta':0.7}).to(device)
    ...
    if itr % test_freq == 0:
        with torch.no_grad():
            pred_y = odeint(func, true_y0, t,atol=1e-3, method='adaptiveLobatto',options={'step_size':0.1,'theta':0.7})
...
#reference-solution:
sol=odeint2(func,true_y0,torch.linspace(0,25,1000,dtype=torch.float64),method="dopri8",atol=1e-12)
torch.save(sol,"results/ref_sol_forward_Lobatto.pt")

sol1=odeint2(func,true_y0,torch.linspace(0,25,1000,dtype=torch.float64),method="adaptiveLobatto",options={'step_size':0.01,'theta':0.7,'conv_ana':True,'file_id':"forward_Lobatto_ada",'max_nodes':max_n})
odeint2(func,true_y0,torch.linspace(0,25,1000,dtype=torch.float64),method="adaptiveLobatto",options={'step_size':0.01,'theta':1.,'conv_ana':True,'file_id':"forward_Lobatto_uni",'max_nodes':max_n})
...
torch.save(sol,"results/ref_sol_backward_Lobatto.pt")


odeint2(augmented_dynamics,aug_state,torch.linspace(25,0,1000,dtype=torch.float64),method="adaptiveLobatto",options={'step_size':0.01,'theta':0.7,'conv_ana':True,'faster_adj_solve':True,"adjoint_params":adjoint_params,"file_id":"backward_Lobatto_ada",'max_nodes':max_n,'original_func':func, 'shapes' :[y0_.shape for y0_ in aug_state]})
odeint2(augmented_dynamics,aug_state,torch.linspace(25,0,1000,dtype=torch.float64),method="adaptiveLobatto",options={'step_size':0.01,'theta':1.,'conv_ana':True,'faster_adj_solve':True,"adjoint_params":adjoint_params,"file_id":"backward_Lobatto_uni",'max_nodes':max_n,'original_func':func, 'shapes' :[y0_.shape for y0_ in aug_state]})
