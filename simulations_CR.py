...
with torch.no_grad():
    true_y = odeint(Lambda(), true_y0, t, atol=1e-2, method='adaptiveCR',options={'step_size':0.1,'theta':0.7,'max_nodes':max_n})
...
for itr in range(1, niters + 1):
    ...
    pred_y = odeint(func, batch_y0, batch_t , method='adaptiveCR',atol=1e-2,faster_adj_solve=True,options={'step_size':0.1,'theta':0.7}).to(device)
    ...
    if itr % test_freq == 0:
        with torch.no_grad():
            pred_y = odeint(func, true_y0, t,atol=1e-2, method='adaptiveCR',options={'step_size':0.1,'theta':0.7})
...
odeint2(augmented_dynamics,aug_state,torch.linspace(25,0,1000,dtype=torch.float64),method="adaptiveCR",options={'step_size':0.01,'theta':0.7,'conv_ana':True,'faster_adj_solve':True,"adjoint_params":adjoint_params,"file_id":"backward_CR_ada",'max_nodes':max_n,'original_func':func, 'shapes' :[y0_.shape for y0_ in aug_state]})
odeint2(augmented_dynamics,aug_state,torch.linspace(25,0,1000,dtype=torch.float64),method="adaptiveCR",options={'step_size':0.01,'theta':1.,'conv_ana':True,'faster_adj_solve':True,"adjoint_params":adjoint_params,"file_id":"backward_CR_uni",'max_nodes':max_n,'original_func':func, 'shapes' :[y0_.shape for y0_ in aug_state]})
